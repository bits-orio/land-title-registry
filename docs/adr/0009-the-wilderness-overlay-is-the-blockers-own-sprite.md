...
> **Amended for map view (0.1.8).** Every blocker cell — wilderness included — draws its striped `-chart` artwork as one chart-mode script sprite, created and destroyed with the blocker through the same refresh paths. The cost objection to per-wilderness-cell objects fell to its own argument: the mod already pays one *entity* per wilderness cell, and one render object per existing blocker is the same cost class. Engine `map_color` tints were tried twice and reverted: recoloring either side of the claimed/unclaimed boundary makes the *other* side read as tinted by contrast, and chart caching means tint changes need a forced rechart to appear at all (the mod now keeps a chart epoch for exactly that). Stripes, never tints; terrain keeps its natural chart colors; Deed stays clean on the map as on the ground.
