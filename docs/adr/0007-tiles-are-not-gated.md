...
Freehold places no collision layer on any tile prototype and sets no flag on one. Landfill, concrete, stone path, and foundation place and mine identically in every cell state, Wilderness included. An earlier design gated them with a fourth collision layer, `fh-cell-tile`, added to every player-placeable tile's mask together with `check_collision_with_entities = true`. That mechanism was cut before implementation.
...
Two further problems compounded it. The engine applies the flag when *mining* a tile as well as when building one, so a cell downgraded to Wilderness would trap its concrete permanently with no way to remove it. And `data-final-fixes.lua` would have had to set the flag on every player-placeable tile in the game, **including modded tiles whose masks Freehold does not control** — for a mod whose stated brand is an empty incompatibility list, the likeliest single source of one.

What the gate bought was thematic: Wilderness stays unpaved. It was never load-bearing. Landfill in a Wilderness cell yields ground the force still cannot build on, because the entity layers do all the real work — the causeway's rails are gated by `fh-transit`, not by the tile.

## Consequences

- The `fh-cell-tile` collision layer does not exist. Three layers ship, all entity-facing: `fh-land`, `fh-transit`, `fh-rampart`.
- The wilderness blocker's mask is `fh-land`, `fh-transit`, `fh-rampart`.
...
- Revisit post-v1 only if playtesting shows unrestricted paving of Wilderness is a real problem, and then only with a mechanism whose blast radius stops at Freehold's own prototypes.
