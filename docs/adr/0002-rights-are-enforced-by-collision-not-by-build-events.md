...
Three custom collision layers (`ltr-land`, `ltr-transit`, `ltr-rampart`) partition every player-creation prototype into exactly one layer at data stage, plus a fourth (`ltr-cell-tile`) gating tile placement. One neutral, indestructible blocker entity per non-Deed cell carries the mask denying the not-yet-earned layers. A Deed cell has no blocker at all — full rights are represented by absence.
...
- Movement is untouched: the blocker masks contain only Land Title Registry's custom layers, no engine movement layers, so characters, vehicles, trains, and enemies traverse every state freely. Only construction is gated.
...
- **Script `create_entity` bypasses collision** unless the caller opts into `build_check_type`. Spawns from other mods sit outside any enforcement model that isn't event policing, and Land Title Registry accepts that.
