...
- Recognition (flying text for the actor, one force-chat line with placement) fires only when the cell has **at least two** entries. The unit of competition is the **force**, not the player: two players sharing a force are one competitor, so celebrations are inherently an MTS-shaped (or otherwise multi-force) feature, and single-force play keeps the quiet personal log without "Fastest!" spam on every deed. `/fh-debug` states which of these conditions is unmet on the cell you are standing on.
- Standings are drawn in the **team's own color with its leader in brackets**, via `mts-v1`'s `get_team_label` — a team reads identically everywhere Freehold names it. No rank-specific palette exists (an earlier gold/silver/bronze scheme was removed for exactly this reason). Labels are re-fetched rather than stored so renames and leader changes stay live, memoized within a tick so a full rebuild costs one call per force.
