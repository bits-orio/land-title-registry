# The `ltr-land-grants` tier ladder is derived from the technology DAG, never hardcoded

Land Title Registry's income chain never names a science pack as a constant. The tier ladder is computed in `data-final-fixes.lua` from the technology graph actually present in the running mod set, with a startup string setting (`ltr-tech-tiers`) as a host override.
...
Since land points are Land Title Registry's only recurring faucet, a dead chain is not a degraded experience; it is an unplayable mod.
...
- `ltr-tech-tiers` (startup, string, default empty) pins the ladder explicitly when the derivation gets a novel tech tree wrong: semicolon-separated tiers, each a comma-separated list of the packs added at that tier. Same philosophy as the layer-membership overrides — the host always has the final word, and no Land Title Registry release is needed to fix a third-party tech tree.
