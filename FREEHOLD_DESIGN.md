...
`fh-refund-percent` is a runtime-global setting, range 0–100, **default 25**. The default must never be high: claiming a cell, exhausting its resources, and downgrading must not be a free roundtrip. Refund arithmetic produces fractional amounts at the default rate (0.5 per Rampart/Deed step, 0.25 per Trail step); balances are therefore plain Lua numbers and fractions are applied exactly — and the remote interface's `get_points` accordingly returns a number, not an integer (see *Interfaces*).
...
- [ ] Remote interface `freehold` (all functions in *Interfaces*, including `reset_force`), custom events `on_cell_claimed` / `on_cell_downgraded` / `on_points_changed` resolvable via `get_event_id`
